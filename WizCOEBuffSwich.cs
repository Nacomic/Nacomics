...
{
    public class WizCOEBuffSwich : BasePlugin, IInGameTopPainter
    {
            ...
            foreach (var player in Hud.Game.Players)
            {
                if (player.IsMe)
                {
                    ...
                    for (int orderIndex = 0; orderIndex < _ruleCalculator.PaintInfoList.Count; orderIndex++)
                    {
                        ...
                        {
                            var skill = player.Powers.UsedSkills.FirstOrDefault(s => s.SnoPower.Sno == Hud.Sno.SnoPowers.Wizard_BlackHole.Sno);
                            if (skill != null)
                                {
                    ...
                    var y = portraitRect.Top + portraitRect.Height * 2.1f;
