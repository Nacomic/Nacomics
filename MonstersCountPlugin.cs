    ...
    public class MonstersCountPlugin : BasePlugin, IInGameTopPainter, IInGameWorldPainter, IKeyEventHandler
    {
        ...
        public void PaintTopInGame(ClipState clipState)
        {
            ...
            if (inRift)
            {
                ...
                if (ElitelocustCount > 0) textBuilder.AppendFormat(" （정예: {0}/{1}）", ElitelocustCount, EliteCount);
                textBuilder.AppendLine();
            }
            if (inRift)
            {
                //textBuilder.AppendFormat("혼출: {0}/{1}", hauntedCount, monstersCount);
                if (ElitehauntedCount > 0) textBuilder.AppendFormat(" （정예: {0}/{1}）", ElitehauntedCount, EliteCount);
                textBuilder.AppendLine();
            }
            ...
            if (palmedCount > 0 && ShowPalmedCount)
            {
                ...
                if (ElitepalmedCount > 0) textBuilder.AppendFormat(" （정예: {0}/{1}）", ElitepalmedCount, EliteCount);
                textBuilder.AppendLine();
            }
